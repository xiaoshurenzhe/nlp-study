...
class DialogueSystem:
    ...
    def load_scenario(self, file):
        ...
        self.all_node_info[REPEAT_NODE['id']] = REPEAT_NODE
        ...
        for slot in node_info.get('slot', []):
            if slot not in memory:
                ...
                if re.search(slot_values, query):
                    ...
                    print('find slot: ', slot, ' with value ', memory[slot])
        return memory
    ...
    def dpo(self, memory):
        if memory["require_slot"] is None:
            ...
            memory["policy"] = "reply"
            hit_node = memory["hit_node"]
            if hit_node == 'noise_node':
                memory['policy'] = 'repeat'
                return memory
            node_info = self.all_node_info[hit_node]
            memory["available_nodes"] = node_info.get("childnode", []) + [REPEAT_NODE['id']]
            print('no required slots, swith to next node ', node_info.get("childnode", []))
        else:
            #有欠缺的槽位
            memory["policy"] = "request"
            memory["available_nodes"] = [memory["hit_node"], REPEAT_NODE['id']] #停留在当前节点
            print('required slot(s) exist(s), remain in current node ', memory["hit_node"])
        return memory
    ...
    def nlg(self, memory):
        ...
        if memory["policy"] == "reply":
            ...
            print('generate reply: ', memory['response'])
        elif memory['policy'] == 'repeat':
            hit_node = memory["hit_node"]
            node_info = self.all_node_info[hit_node]
            memory["response"] = node_info['response']
            print('generate request: ', memory['response'])
        else:
            #policy == "request"
...
if __name__ == '__main__':
    ...
    memory = {"available_nodes": ["scenario-买衣服_node1", "scenario-看电影_node1", "noise_node"]}  #用户状态
    while True:
        query = input("请输入：")
        # query = "你好"    
        ...
        print(response)
        print("===========")
        if memory['available_nodes'] == ['noise_node']:
            memory = {"available_nodes": ["scenario-买衣服_node1", "scenario-看电影_node1", "noise_node"]}
