...
class GraphQA:
    def __init__(self):
        self.graph = Graph("http://localhost:7474", auth=("neo4j", "huge"))
...
if __name__ == "__main__":
    graph = GraphQA()
    res = graph.query("谁导演的繁花")
    print(res)
    res = graph.query("六月的雨的谱曲是谁")
    print(res)
    res = graph.query("胡歌的民族是什么")
    print(res)
    res = graph.query("仙剑奇侠传的女主演是") # 没有找到答案
    print(res)
    res = graph.query("仙剑奇侠传三的编剧是谁")
    print(res)
    res = graph.query("刘亦菲和刘涛是什么关系")
    print(res)
    res = graph.query("刘亦菲和仙剑奇侠传是什么关系")
    print(res)
