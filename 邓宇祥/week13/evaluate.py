...
class Evaluator:
    ...
    def eval(self, epoch):
        ...
        for index, batch_data in enumerate(self.valid_data):
            ...
            input_ids, labels = batch_data['input_ids'], batch_data['labels']  #输入变化时这里需要修改，比如多输入，多输出的情况
            with torch.no_grad():
                pred_results = self.model(input_ids=input_ids,attention_mask=batch_data["attention_mask"]) #不输入labels，使用模型当前参数进行预测
